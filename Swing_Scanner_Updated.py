...
Requirements:
    pip install yfinance pandas talib-binary requests rich feedparser pytz openpyxl
    Note: If talib-binary fails try: pip install TA-Lib
...
SR_ZONE_PCT    = 0.015
RISK_REWARD    = 2.0
MIN_SCORE      = 7    # Raised from 8 → only best setups
...
def get_nifty_trend() -> str:
    try:
        ...
        c     = float(last["Close"])
        if c > ema20 > ema50:   return "bullish"
        elif c < ema20 < ema50: return "bearish"
        else:                   return "neutral"
    except Exception:
        return "neutral"
...
def analyze_stock(symbol: str, nifty_trend: str) -> dict | None:
    ...
    try:
        ...
        df["RSI"]       = talib.RSI(close_arr, timeperiod=RSI_PERIOD)
        _, _, macdh     = talib.MACD(close_arr, fastperiod=12, slowperiod=26, signalperiod=9)
        df["MACDh"]     = macdh
        df["MACDh_prev"]= pd.Series(macdh, index=df.index).shift(1)
        bbu, _, bbl     = talib.BBANDS(close_arr, timeperiod=BB_PERIOD,
                                       nbdevup=BB_STD, nbdevdn=BB_STD)
        df["BB_Upper"]  = bbu
        df["BB_Lower"]  = bbl
        df["EMA_Short"] = talib.EMA(close_arr, timeperiod=EMA_SHORT)
        df["EMA_Long"]  = talib.EMA(close_arr, timeperiod=EMA_LONG)
        df["ADX"]       = talib.ADX(high_arr, low_arr, close_arr, timeperiod=ADX_PERIOD)
        df["ATR"]       = talib.ATR(high_arr, low_arr, close_arr, timeperiod=ATR_PERIOD)
